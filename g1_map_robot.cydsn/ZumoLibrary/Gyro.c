/**
 * @file    Gyro.c
 * @brief   Basic methods for operating gyroscope. For more details, please
 * refer to Gyro.h file.
...
void
writeReg (uint8 reg, uint8 value)
{
...
enableDefault (void)
{
  ...
  // 0x6F = 0b01101111
  // DR = 01 (200 Hz ODR); BW = 10 (50 Hz bandwidth); PD = 1 (normal mode); Zen
  // = Yen = Xen = 1 (all axes enabled)
  writeReg (CTRL_REG1, 0x6F);
}

uint16
value_convert_gyro (uint16 raw)
{
  float rate_result;
