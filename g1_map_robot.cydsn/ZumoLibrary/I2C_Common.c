/**
 * @file    I2C_made.c
 * @brief   Basic methods for I2C communication. For more details, please refer
 * to I2C_made.h file.
 * @details
 */
 ...
 */
uint8_t
I2C_Write (uint8_t device, uint8_t regAdd, uint8_t data)
{

  uint8 write_buf[2] = { regAdd, data };

  I2C_MasterClearStatus (); // clear any pending status flags
  I2C_MasterWriteBuf (device, write_buf, 2,
                      I2C_MODE_COMPLETE_XFER); // one complete byte transfer
  while (((I2C_MasterStatus ()) & I2C_MSTAT_WR_CMPLT) == 0)
    ; // Check status and make sure it has completed
  return 0;
}
...
uint8_t
I2C_Read (uint8_t device, uint8_t regAdd)
{
 ...
 * @param    uint8 device : Slave Device address
 * @param    uint8 startAdd : The address of the first register to be read.
 * Subsequent reads will auto increment this
 * @param    uint8 data : Pointer for value/values to be read
 * @param    uint8 num_bytes_to_be_read : number of bytes to be read
 */

void
I2C_Read_Multiple (uint8_t device, uint8_t startAdd, uint8_t *data,
                   uint8_t num_bytes_to_be_read)
{

  I2C_MasterClearStatus (); // clear any pending status flags
  I2C_MasterWriteBuf (device, &startAdd, 1,
                      I2C_MODE_COMPLETE_XFER); // one complete byte transfer to
                                               // establish address to be read
  while ((I2C_MasterStatus () & I2C_MSTAT_WR_CMPLT) == 0)
    ;                       // Check status and make sure it has completed
  I2C_MasterClearStatus (); // clear any pending status flags
  I2C_MasterReadBuf (
      device, data, num_bytes_to_be_read,
      I2C_MODE_COMPLETE_XFER); //  complete byte/multi-byte receive
