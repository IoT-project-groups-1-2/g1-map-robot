/**
 * @file IR.c
 * @brief Basic methods for operating IR receiver. For more details, please
 * refer to IR.h file.
  ...
  if (status & Timer_IR_STATUS_CAPTURE)
    {
      ...
      if (IR_receiver_Read () == 0)
        { // signal was low --> we captured falling edge
...
bool
IR_get (uint32_t *value, uint32_t wait)
{
