/**
 * @file    Reflectance.c
 * @brief   Basic methods for operating reflectance sensor. For more details,
 * please refer to Reflectance.h file.
...
static struct sensors_ threshold
 ...
 */
void
ReflectanceTask (void *pvParameters)
{
  (void)pvParameters;
  ...
  if (ReflectanceHandle == NULL)
    {
      ...
      Timer_L2_Start ();
      Timer_L1_Start ();
      // task runs at highest priority and wakes up every millisecond but uses
      // very little CPU time
      xTaskCreate (ReflectanceTask, "Reflectance", configMINIMAL_STACK_SIZE,
                   NULL, tskIDLE_PRIORITY + 3, &ReflectanceHandle);
    }
}
 ...
 */
void
reflectance_read (struct sensors_ *values)
{
  *values = sensors;
}
 ...
 */
void
reflectance_digital (struct sensors_ *digital)
{
  // if the results of reflectance_period function is over threshold, set
  // digital_sensor_value to 1, which means it's black
...
void
reflectance_set_threshold (uint16_t L3, uint16_t L2, uint16_t L1, uint16_t R1,
