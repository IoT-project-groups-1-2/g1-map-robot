...
void
DebugUartTask (void *pvParameters)
{
  ...
  while (*s)
    {
      if (*s == '\n')
        {
          const char cr = '\r';
          xQueueSend (xSerialTxQueue, &cr,
                      portMAX_DELAY); // UART_1_PutChar('\r');
        }
      xQueueSend (xSerialTxQueue, s++,
                  portMAX_DELAY); // UART_1_PutChar(*ptr++);
    }
}

#define CMD_MAX_SIZE 64
void
DebugCommandTask (void *pvParameters)
{
  (void)pvParameters; // quiet the compiler warning
  char c;
