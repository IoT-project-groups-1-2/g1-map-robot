...
// macro for changing state. Correct operation requires that ctx is a pointer
// to state machine instance (see state template)
...
typedef void (*smf) (
    smi *, const event *); // prototype of state handler function pointer
...
static inline void
serial_write_buf (smi *ctx, const char *buf, int len)
{
  while (len-- > 0)
    {
...
static inline void
serial_write_str (smi *ctx, const char *s)
{
...
int
esp_socket (const char *ssid, const char *password)
{
  ...
  (void)xTaskCreate (vEspSocketTask, "ESP_sock", configMINIMAL_STACK_SIZE * 6,
                     &EspSocketInstance, tskIDLE_PRIORITY + 2, NULL);
  (void)xTaskCreate (vEspEventTask, "ESP_rxe", configMINIMAL_STACK_SIZE,
                     &EspSocketInstance, tskIDLE_PRIORITY + 2, NULL);
  (void)xTaskCreate (vEspTickTask, "ESP_tick", configMINIMAL_STACK_SIZE,
                     &EspSocketInstance, tskIDLE_PRIORITY + 2, NULL);
...
int
esp_connect (int sockfd, const char *addr, int port)
{
  I_DONT_USE (sockfd);
  ...
  int rc = -1;
  if (xSemaphoreTake (EspSocketInstance.Access, configTICK_RATE_HZ * 3)
      == pdTRUE)
    {
...
int
esp_read (int sockfd, void *data, int length)
{
  I_DONT_USE (sockfd);
  int count = 0;

  if (xSemaphoreTake (EspSocketInstance.Access, configTICK_RATE_HZ / 10)
      == pdTRUE)
    {
      // read data
...
int
esp_write (int sockfd, void *data, int length)
{
  I_DONT_USE (sockfd);
...
int
esp_close (int sockfd)
{
  return esp_shutdown (sockfd, -1);
}

int
esp_shutdown (int sockfd, int how)
{
  I_DONT_USE (sockfd);
  I_DONT_USE (how);
 ...
 * Returns true when specified amount of characters have been accumulated. */
bool
sm_read_buffer (smi *ctx, int count)
{
  while (ctx->pos < (SMI_BUFSIZE - 1) && ctx->pos < count
         && serial_get (ctx, ctx->buffer + ctx->pos))
    {
...
/* Read an integer.
 * Consumes characters until a non-nondigit is received. The nondigit is also
 * consumed. */
bool
sm_read_int (smi *ctx, int *value)
{
  bool result = false;
  while (ctx->pos < (SMI_BUFSIZE - 1)
         && serial_get (ctx, ctx->buffer + ctx->pos))
    {
...
int
sm_read_until (smi *ctx, const char **p)
{
  int result = RC_NOT_AVAILABLE;
  while (result < 0 && ctx->pos < (SMI_BUFSIZE - 1)
         && serial_get (ctx, ctx->buffer + ctx->pos))
    {
      ++ctx->pos;
      ctx->buffer[ctx->pos] = '\0';
...
bool
sm_wait_for (smi *ctx, const char *p)
{
  bool result = false;
  int len = strlen (p);
...
void
stEchoOff (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
void
stStationModeCheck (smi *ctx, const event *e)
{
  int rc = -1;
  switch (e->ev)
    {
...
void
stStationModeSet (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
void
stConnectAP (smi *ctx, const event *e)
{
  int rc;
  switch (e->ev)
    {
...
void
stConnectTCP (smi *ctx, const event *e)
{
  int rc;
  switch (e->ev)
    {
...
void
stPassthrough (smi *ctx, const event *e)
{
  int rc;
  switch (e->ev)
    {
...
void
stPassthroughOK (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
#if 1
void
stConnected (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
void
stCommandMode (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
void
stCloseTCP (smi *ctx, const event *e)
{
  int rc = -1;
  switch (e->ev)
    {
...
void
stReceiveLength (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
void
stReceiveData (smi *ctx, const event *e)
{
  switch (e->ev)
    {
    case eEnter:
...
static void
dispatch_event (smi *ctx, const event *e)
{
...
void
EspTimerCallback (TimerHandle_t xTimer)
{
  I_DONT_USE (xTimer);
}
...
void
vEspTickTask (void *pvParameters)
{
...
void
vEspEventTask (void *pvParameters)
{
  QueueHandle_t eq = ((smi *)pvParameters)->EspEventQ;
  ...
  while (true)
    {
      ...
      if (xSerialWaitForRxEvent (port, 50) == pdTRUE)
        ...
        {
          // if nothing was received but there is data in the buffers then send
 ...
 */
void
vEspSocketTask (void *pvParameters)
{
