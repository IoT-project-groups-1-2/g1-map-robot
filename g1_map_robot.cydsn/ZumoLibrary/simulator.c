...
_write (int file, char *ptr, int len)
{
  ...
  if (xSemaphoreTake (writeMutex, portMAX_DELAY) == pdFALSE)
    return 0;
...
int
_read (int file, char *ptr, int count)
{
  (void)file;
  (void)ptr;
  (void)count;
...
void
SimulatorTask (void *pvParameters)
{
  (void)pvParameters;
